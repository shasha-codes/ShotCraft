...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // The Mantle bearer key belongs to a separate AWS account. Keep it in
    // Secrets Manager in this deployment account and let only the runtime role
    // retrieve it. The secret value never enters source control or stack config.
    const bedrockApiKey = secretsmanager.Secret.fromSecretNameV2(
      this,
      'ShotCraftBedrockApiKey',
      'shotcraft/bedrock-api-key'
    );
    for (const env of this.application.environments.values()) {
      env.runtime.addEnvironmentVariable('SHOTCRAFT_BEDROCK_SECRET_ARN', bedrockApiKey.secretArn);
      env.runtime.addEnvironmentVariable('SHOTCRAFT_BEDROCK_ENDPOINT', 'https://bedrock-mantle.us-west-2.api.aws/v1');
      env.runtime.addEnvironmentVariable('SHOTCRAFT_OPENAI_MODEL', 'openai.gpt-oss-120b');
      bedrockApiKey.grantRead(env.runtime.role);
    }
