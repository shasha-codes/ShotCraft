...
clientOverview=()=>{const originalShoots=shoots,dateValue=record=>{const value=record.call_time||payload(record).shoot_date,timestamp=value?wallClockDate(value).valueOf():NaN;return Number.isFinite(timestamp)?timestamp:Number.MAX_SAFE_INTEGER},scheduled=[...shoots].filter(record=>record.status==='SCHEDULED').sort((a,b)=>dateValue(a)-dateValue(b)||b.id-a.id);shoots=[...shoots].sort((a,b)=>dateValue(a)-dateValue(b));clientOverviewWithDateSort();const hero=app.querySelector('.overview-next'),heading=app.querySelector('.overview-list-head'),list=app.querySelector('.overview-recent');if(hero&&scheduled[0])hero.outerHTML=clientOverviewNextShoot(scheduled[0]);if(heading){const eyebrow=heading.querySelector('.eyebrow'),title=heading.querySelector('h2'),link=heading.querySelector('[data-view="inquiries"]');if(eyebrow)eyebrow.textContent='Coming up';if(title)title.textContent='My scheduled shoots';if(link)link.textContent='View all shoots →'}if(list){list.className='overview-projects client-scheduled-projects';list.innerHTML=scheduled.slice(0,2).map((record,index)=>galleryCard(record,index+1)).join('')||'<div class="card empty">No scheduled shoots yet.</div>'}shoots=originalShoots;bind();decorateClientUnread()};
