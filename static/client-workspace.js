const editorialTheme=document.createElement('link');editorialTheme.rel='stylesheet';editorialTheme.href='/static/editorial-theme.css?v=sidebar-signout-1';document.head.appendChild(editorialTheme);
...
function notificationIcon(type){return ({FOLLOWUP_REQUESTED:'?',FOLLOWUP_COMPLETE:'✓',PLAN_READY:'✦',SCHEDULE_OPTIONS:'◷',SHOOT_CONFIRMED:'✓',SHOOT_CANCELLED:'×',CANCELLATION_DECLINED:'↩'})[type]||'•'}
...
function arrangeClientPlanDecision(){
  ...
  if(policy&&approve)schedule.insertBefore(policy,approve);
