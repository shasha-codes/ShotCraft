...
async function clientMessageInbox(){
  ...
  try{
    ...
    const conversations=shoots.filter(record=>Number(record.message_count||0)>0).sort((a,b)=>String(b.latest_message?.created_at||'').localeCompare(String(a.latest_message?.created_at||'')));
...
async function submitShootChange(event,id){
  ...
  try{
    ...
    await refresh();current={view:'details',id:Number(id)};history.pushState(current,'',`/client?view=details&id=${id}`);galleryDetail(id);setTimeout(()=>{document.querySelector('.shoot-change-toast')?.remove();const toast=document.createElement('div');toast.className='shoot-change-toast';toast.textContent='Change request sent. Your photographer will review it.';document.body.append(toast);setTimeout(()=>toast.remove(),4200)},50);
...
galleryDetail=async id=>{await galleryDetailWithShootEdit(id);const actions=app.querySelector('.detail-next .actions');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};
const cinematicPlanWithShootEdit=cinematicPlan;
cinematicPlan=async id=>{await cinematicPlanWithShootEdit(id);const actions=app.querySelector('.plan-decision');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};
...
const finalGalleryDetailWithAmPmClock=galleryDetail;
galleryDetail=async id=>{
  await finalGalleryDetailWithAmPmClock(id);
  const record=await getRecord(id);
  if(!record.call_time)return;
  const time=[...app.querySelectorAll('.detail-facts>div')].find(item=>item.querySelector('span')?.textContent.includes('Time'))?.querySelector('b');
  if(time)time.textContent=wallClockDate(record.call_time).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit',hour12:true});
};
const finalCinematicPlanWithAmPmClock=cinematicPlan;
cinematicPlan=async id=>{
  await finalCinematicPlanWithAmPmClock(id);
  const record=await getRecord(id);
  if(!record.call_time)return;
  const field=[...app.querySelectorAll('.call-grid>div')].find(item=>item.querySelector('span')?.textContent.trim().toLowerCase()==='time');
  if(field?.querySelector('b'))field.querySelector('b').textContent=wallClockDate(record.call_time).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit',hour12:true});
...
// Keep the client schedule-change route and entry points last in this legacy file.
const latestRenderWithShootEdit=render;
render=()=>current.view==='edit-shoot'?editShoot(current.id):latestRenderWithShootEdit();
const latestGalleryWithShootEdit=galleryDetail;
galleryDetail=async id=>{await latestGalleryWithShootEdit(id);const actions=app.querySelector('.detail-next .actions');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};
const latestPlanWithShootEdit=cinematicPlan;
cinematicPlan=async id=>{await latestPlanWithShootEdit(id);const actions=app.querySelector('.plan-decision');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};

// Final wrappers ensure the schedule-change route survives legacy view enhancers.
const finalRenderWithShootEdit=render;
render=()=>current.view==='edit-shoot'?editShoot(current.id):finalRenderWithShootEdit();
const finalGalleryWithShootEdit=galleryDetail;
galleryDetail=async id=>{await finalGalleryWithShootEdit(id);const actions=app.querySelector('.detail-next .actions');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};
const finalPlanWithShootEdit=cinematicPlan;
cinematicPlan=async id=>{await finalPlanWithShootEdit(id);const actions=app.querySelector('.plan-decision');if(actions&&!actions.querySelector('[data-edit-shoot]'))actions.insertAdjacentHTML('beforeend',`<button class="secondary" data-edit-shoot="${id}">Edit shoot date or time</button>`)};
