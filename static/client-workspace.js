...
async function refresh(){const response=await fetch('/api/inquiries?client_email='+encodeURIComponent(clientUser.email||''),{cache:'no-store'});const data=await response.json();if(response.status===401){sessionStorage.removeItem('shotcraftUser');location.replace('/auth?mode=login');throw Error('Your session has expired. Please sign in again.')}if(!response.ok||!Array.isArray(data))throw Error(data?.detail||'Could not load your shoots.');const dateValue=record=>{const p=payload(record),value=record.call_time||p.shoot_date,timestamp=value?Date.parse(String(value).replace(' ','T')):NaN;return Number.isFinite(timestamp)?timestamp:Number.MAX_SAFE_INTEGER};shoots=data.sort((a,b)=>dateValue(a)-dateValue(b)||b.id-a.id)}
function galleryCard(record,index=0){const p=payload(record),status=statusInfo(record),city=(p.message||'').match(/Seattle|Delhi|New Delhi/i)?.[0]?.toLowerCase().replace('new delhi','delhi'),gallery=cityImages[city]?.gallery||[],mood=parsed(record.moodboard),moodImage=(mood?.generated?.tiles||[]).find(tile=>tile?.image_url)?.image_url,image=moodImage||gallery[index%Math.max(gallery.length,1)]||null,location=record.meeting_location||p.location||(city==='delhi'?'Delhi, India':city==='seattle'?'Seattle, Washington':'Location to be confirmed');return`<article class="gallery-card ${index===0?'gallery-featured':''} ${image?'gallery-has-image':'gallery-no-image'}">${image?`<img src="${image}" alt="${esc(location)} creative reference" loading="lazy">`:''}<div class="gallery-shade"></div><button class="circle-action" data-detail="${record.id}" aria-label="View ${esc(title(record))}">→</button><div class="gallery-content"><h2>${esc(title(record))}</h2><div class="gallery-meta"><span>▣ ${esc(formatDate(p.shoot_date))}</span><span>⌖ ${esc(location)}</span></div><span class="status ${status[1]}">${status[0]}</span>${index===0?`<button class="primary gallery-cta" data-detail="${record.id}">View shoot details →</button>`:`<button class="link gallery-link" data-detail="${record.id}">View details →</button>`}</div></article>`}
...
const creativePreviewMarkup=(images,kind='moodboard')=>`<span class="creative-preview-label"><i></i>${kind==='moodboard'?'Creative direction':'Location inspiration'}</span>${images.map((image,index)=>`<img src="${esc(image)}" alt="${kind==='moodboard'?'Creative direction':'Location'} reference ${index+1}">`).join('')}`;
const creativePendingMarkup=()=>'<div class="creative-pending-grid" aria-hidden="true"><span></span><span></span><span></span><span></span></div><div class="creative-pending-copy"><i aria-hidden="true"></i><b>Shaping your creative direction</b><span>Your photographer’s visual references will appear here shortly.</span></div>';
...
clientProfile=()=>{setNav('profile');app.innerHTML=`<div class="head"><div><div class="eyebrow">Your profile</div><h1>Keep your details current.</h1><p class="lede">Photographers use this information to understand who they’re planning with.</p></div></div><form class="card profile-form" id="clientProfileForm"><div class="profile-picture-field"><div class="profile-picture-preview">${clientUser.profile_image?`<img src="${esc(clientUser.profile_image)}" alt="Profile picture">`:'<span>◎</span>'}</div><div><label for="profileImage">Profile picture</label><input id="profileImage" type="file" accept="image/png,image/jpeg,image/webp"><small class="muted">PNG, JPEG, or WebP · 2 MB maximum</small></div></div><div class="field"><label for="profileName">Name</label><input id="profileName" value="${esc(clientUser.name||'')}" readonly></div><div class="field"><label for="profileEmail">Email address</label><input id="profileEmail" value="${esc(clientUser.email||'')}" disabled></div><div class="field"><label for="profileCity">City</label><input id="profileCity" value="${esc(clientUser.city||'')}" placeholder="e.g. Seattle, WA" maxlength="120"></div><div class="field"><label for="profileBio">About you <span>optional</span></label><textarea id="profileBio" maxlength="500" placeholder="A little context about you or the kind of work you do.">${esc(clientUser.bio||'')}</textarea></div><div class="actions"><button class="primary">Save profile →</button><span class="muted" id="profileStatus"></span></div></form>`;document.getElementById('clientProfileForm').onsubmit=saveClientProfile};
...
/* Time options arrive with the shared production plan, then the client approves
...
cinematicPlan=async id=>{
  ...
  if(!decision||!record.production_approved||['CLIENT_CONFIRMED','SCHEDULED'].includes(record.status))return;
  ...
  if(!decision.querySelector('.shared-time-summary'))decision.insertAdjacentHTML('afterbegin',`<div class="shared-time-summary"><b>Times proposed by your photographer</b>${request.suggestions.map(slot=>`<span>${esc(calendarDate(slot.starts_at))} · ${esc(calendarTime(slot.starts_at))}–${esc(calendarTime(slot.ends_at))}</span>`).join('')}</div>`);
};
document.addEventListener('click',async event=>{const button=event.target.closest('[data-confirm-and-choose]');if(!button)return;event.preventDefault();button.disabled=true;try{const response=await fetch(`/api/inquiries/${button.dataset.confirmAndChoose}/client-confirm`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})}),data=await response.json();if(!response.ok||data.error)throw Error(data.detail||data.error||'Could not approve this plan.');await refresh();go('schedule-options',button.dataset.confirmAndChoose)}catch(error){button.disabled=false;button.textContent=error.message}});
...
function showFollowupSubmittedToast(done){
  document.querySelector('.followup-success-toast')?.remove();
  const toast=document.createElement('div');
  toast.className='followup-success-toast';
  toast.setAttribute('role','status');
  toast.innerHTML='<i>✓</i><div><strong>Follow-up answers submitted</strong><span></span></div>';
  document.body.appendChild(toast);
  const message=toast.querySelector('span');
  let remaining=3;
  const update=()=>message.textContent=`Your photographer has everything they need. Taking you to Overview in ${remaining}…`;
  update();
  const countdown=setInterval(()=>{remaining-=1;if(remaining>0)update()},1000);
  setTimeout(()=>{clearInterval(countdown);toast.remove();done()},3000);
}
...
reply=async(event,id)=>{
  ...
  try{
    ...
    showFollowupSubmittedToast(()=>go('shoots'));
    refresh().catch(()=>{});
  }catch(error){
...
create=()=>{
  ...
  picker.innerHTML=windows.map(([label,start,end])=>`<button type="button" class="time-window" data-time-window data-start="${start}" data-end="${end}"><b>${label}</b><span>${start}–${end}</span></button>`).join('');
...
cinematicPlan=async id=>{
  ...
  const record=await getRecord(id);
  if(!record.production_approved||['SCHEDULED','CLIENT_CHANGE_REQUESTED'].includes(record.status))return;
  const response=await fetch(`/api/inquiries/${id}/schedule-recommendations`,{cache:'no-store'}),request=response.ok?await response.json():null;
  ...
  const panel=`<section class="inline-schedule"><header><b>Choose your preferred time</b><span>Select one of the photographer’s available options, then approve your plan.</span></header><div class="inline-schedule-options">${request.suggestions.map(slot=>`<button type="button" class="inline-schedule-option" data-inline-time-choice data-start="${esc(slot.starts_at)}" data-end="${esc(slot.ends_at)}" data-location="${esc(slot.location)}"><span><b>${esc(calendarDate(slot.starts_at))}</b><small>${esc(calendarTime(slot.starts_at))} – ${esc(calendarTime(slot.ends_at))}${slot.location?` · ${esc(slot.location)}`:''}</small></span><i>→</i></button>`).join('')}</div><button type="button" class="primary" data-approve-plan-time="${id}">Approve plan & selected time →</button><p class="inline-schedule-status" id="inlineScheduleStatus">Choose a time to continue.</p></section>`;
...
document.addEventListener('click',async event=>{
  ...
  if(choice){
    ...
    panel.querySelectorAll('[data-inline-time-choice]').forEach(option=>option.classList.toggle('selected',option===choice));
    ...
    panel.querySelector('#inlineScheduleStatus').textContent='Time selected. Approve the plan when you’re ready.';
    return;
  }
  ...
  button.disabled=true;status.textContent='Approving your plan and time…';
  try{
    const record=await getRecord(button.dataset.approvePlanTime);
    if(record.status!=='CLIENT_CONFIRMED'){
      const approval=await fetch(`/api/inquiries/${button.dataset.approvePlanTime}/client-confirm`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})}),approvalData=await approval.json();
      if(!approval.ok||approvalData.error)throw Error(approvalData.detail||approvalData.error||'Could not approve the production plan.');
    }
...
galleryDetail=async id=>{
  ...
  const signature=record=>`${record.status}|${Boolean(record.production_approved)}|${Boolean(record.production_pack)}`;
...
clientCreativePreviewFraming.textContent='@media(min-width:701px){.detail-strip.client-creative-preview{grid-template-columns:1.45fr repeat(3,minmax(0,1fr))!important;grid-template-rows:1fr!important;min-height:330px}.client-creative-preview>img{grid-row:auto!important;min-height:330px!important;object-fit:contain!important;object-position:center!important;padding:4px;background:#211f25}}';
