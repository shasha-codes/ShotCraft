const editorialTheme=document.createElement('link');editorialTheme.rel='stylesheet';editorialTheme.href='/static/editorial-theme.css?v=client-status-2';document.head.appendChild(editorialTheme);
...
clientOverview=()=>{const originalShoots=shoots,dateValue=record=>{const value=record.call_time||payload(record).shoot_date,timestamp=value?wallClockDate(value).valueOf():NaN;return Number.isFinite(timestamp)?timestamp:Number.MAX_SAFE_INTEGER},scheduled=[...shoots].filter(record=>record.status==='SCHEDULED').sort((a,b)=>dateValue(a)-dateValue(b)||b.id-a.id);shoots=[...shoots].sort((a,b)=>dateValue(a)-dateValue(b));clientOverviewWithDateSort();const heading=app.querySelector('.overview-list-head'),list=app.querySelector('.overview-recent');if(heading){const eyebrow=heading.querySelector('.eyebrow'),title=heading.querySelector('h2'),link=heading.querySelector('[data-view="inquiries"]');if(eyebrow)eyebrow.textContent='Coming up';if(title)title.textContent='My scheduled shoots';if(link)link.textContent='View all shoots →'}if(list)list.innerHTML=scheduled.slice(0,3).map(record=>{const itemStatus=statusInfo(record);return`<button class="overview-recent-row" data-detail="${record.id}"><span class="overview-recent-status ${itemStatus[1]}">${itemStatus[0]}</span><span><b>${esc(title(record))}</b><small>${esc(formatDate(record.call_time))}</small></span><span class="overview-row-arrow">→</span></button>`}).join('')||'<div class="card empty">No scheduled shoots yet.</div>';shoots=originalShoots;bind()};
...
cinematicPlan=async id=>{
  ...
  if(!decision.querySelector('.booking-policy'))decision.querySelector('.shared-time-summary')?.insertAdjacentHTML('afterend',`<section class="booking-policy"><b>Cancellation policy</b><span>${esc(bookingPolicySummary)}</span><label><input type="checkbox" data-policy-accept="${id}"> I understand and accept the cancellation policy.</label></section>`);
};
...
function showFollowupSubmittedToast(){
  ...
  toast.innerHTML='<i>✓</i><div><strong>Follow-up answers submitted</strong><span>Your photographer is preparing your creative plan. We’ll notify you when your next decision is ready.</span></div>';
