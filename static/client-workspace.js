...
create=()=>{
  createWithEditorialLayout();enhanceNewInquiryLayout();
  const date=document.getElementById('date');
  if(date){date.min=localToday();date.addEventListener('focus',()=>{date.min=localToday()})}
};
...
submitInquiry=async event=>{
  ...
  const date=document.getElementById('date');
  date.min=localToday();
  if(date.value<date.min){date.setCustomValidity('Choose today or a future date.');date.reportValidity();status.textContent='Choose today or a future shoot date.';date.addEventListener('input',()=>date.setCustomValidity(''),{once:true});return}
  if(!windows.length){status.textContent='Select at least one preferred time window.';return}
  if(!city){status.textContent='Select a city from the recommendations.';return}
  button.disabled=true;status.textContent='Creating your draft shoot plan…';
  try{
